...
export const ChatLauncher: React.FC<ChatLauncherProps> = ({ isOpen, onClick, unreadCount }) => {
    return (
        <button
            onClick={onClick}
            className={`fixed bottom-20 right-6 w-14 h-14 rounded-full bg-primary text-primary-foreground shadow-lg flex items-center justify-center transition-all duration-300 z-[10000] hover:scale-105 ${!isOpen && unreadCount > 0 ? 'animate-pulse' : ''
                }`}
